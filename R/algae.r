...
algae <- function(data) {
  ...
  microalgae <- data.frame(cbind(
    data$id[which(data$AnalyteName == 'Microalgae Thickness')],
    as.character(data$VariableResult[which(
      data$AnalyteName == 'Microalgae Thickness'
    )])
  ))
  ...
  PCT_MIATP.result <- round(tapply(
    microalgae$VariableResult,
    microalgae$id,
    FUN_PCT_MIATP
  ))
  PCT_MIATP.count <- tapply(
    microalgae$VariableResult,
    microalgae$id,
    function(x) {
  ...
  PCT_MIAT1.result <- round(tapply(
    microalgae$VariableResult,
    microalgae$id,
    FUN_PCT_MIAT1
  ))
  PCT_MIAT1.count <- tapply(
    microalgae$VariableResult,
    microalgae$id,
    function(x) {
      return(sum(x %in% c('0', '1', '2', '3', '4', '5')))
  ...
  PCT_MIAT1P.result <- round(tapply(
    microalgae$VariableResult,
    microalgae$id,
    FUN_PCT_MIAT1P
  ))
  PCT_MIAT1P.result[is.na(PCT_MIAT1P.result)] <- 0
  PCT_MIAT1P.count <- tapply(
    microalgae$VariableResult,
    microalgae$id,
    function(x) {
  ...
  XMIAT_meanst <- round(
    tapply(XMIAT_frame$result, XMIAT_frame$id, XMIAT_meanss),
    1
  )
  ...
  XMIATP_meanst <- round(
    tapply(XMIAT_frame$result, XMIAT_frame$id, XMIATP_meanss),
    1
  )
  ...
  macrophyte_cover <- data.frame(cbind(
    data$id[which(data$AnalyteName == 'Macrophyte Cover')],
    as.character(data$VariableResult[which(
      data$AnalyteName == 'Macrophyte Cover'
    )])
  ))
  ...
  PCT_MCP.result <- tapply(
    macrophyte_cover$VariableResult,
    macrophyte_cover$id,
    PCT_MCP_stats
  ) %>%
    round
  PCT_MCP.count <- tapply(
    macrophyte_cover$VariableResult,
    macrophyte_cover$id,
    function(data) {
      present <- length(which(data == "Present"))
  ...
  macroalgae_cover_attached <- data.frame(cbind(
    data$id[which(data$AnalyteName == 'Macroalgae Cover, Attached')],
    as.character(data$VariableResult[which(
      data$AnalyteName == 'Macroalgae Cover, Attached'
    )])
  ))
  ...
  PCT_MAA.result <- tapply(
    macroalgae_cover_attached$VariableResult,
    macroalgae_cover_attached$id,
    PCT_MAA_stats
  ) %>%
    round
  PCT_MAA.count <- tapply(
    macroalgae_cover_attached$VariableResult,
    macroalgae_cover_attached$id,
    function(data) {
      present <- length(which(data == "Present"))
  ...
  macroalgae_cover_unattached <- data.frame(cbind(
    data$id[which(data$AnalyteName == 'Macroalgae Cover, Unattached')],
    as.character(data$VariableResult[which(
      data$AnalyteName == 'Macroalgae Cover, Unattached'
    )])
  ))
  ...
  PCT_MAU.result <- tapply(
    macroalgae_cover_unattached$VariableResult,
    macroalgae_cover_unattached$id,
    PCT_MAA_stats
  ) %>%
    round
  PCT_MAU.count <- tapply(
    macroalgae_cover_unattached$VariableResult,
    macroalgae_cover_unattached$id,
    function(data) {
      present <- length(which(data == "Present"))
  ...
  PCT_MAP <- data %>%
    ...
    dplyr::mutate(
      ...
      PCT_MAP.result = purrr::map(data, function(df) {
        number_present <- df[df$VariableResult == 'Present', ]$LocationCode %>%
          unique %>%
          length
